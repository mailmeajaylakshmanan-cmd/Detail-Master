...
// Aggregate stats for the dashboard — micro-cached with 30s TTL
router.get('/stats', async (req, res) => {
  try {
    const stats = await queryCache.getOrSet('dashboard_stats', async () => {
      const [revRes, cntRes, todayRes, clientRes, orgRes, recentRes, mixRes, scheduleRes] = await Promise.all([
        ...
        `),
        db.query('SELECT COUNT(*)::int AS total FROM clients WHERE is_active = true'),
        db.query('SELECT COUNT(*)::int AS total FROM organizations WHERE is_active = true'),
        db.query(`
          SELECT 
