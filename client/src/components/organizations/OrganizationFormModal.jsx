...
export default function OrganizationFormModal({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex items-end sm:items-center justify-center z-[100] p-0 sm:p-4 overflow-y-auto">
      <div className="w-full max-w-2xl bg-white/95 backdrop-blur-2xl shadow-2xl border border-white/60 rounded-t-[28px] sm:rounded-[24px] flex flex-col max-h-[92vh] sm:max-h-[85vh] overflow-hidden animate-scale-up">
        {/* Sticky Header */}
        <div className="flex justify-between items-center px-5 sm:px-6 py-4 sm:py-5 border-b border-gray-100/60 bg-white/60 shrink-0">
          <div>
            ...
            <p className="text-[11px] font-bold text-gray-400">
              Corporate account details & fleet registration
            </p>
          </div>
            ...
            <div className="pt-2">
              ...
              <div className="space-y-3">
                {formVehicles.map((v, idx) => {
                  ...
                  return (
                    <div key={v.id || `new-${idx}`} className="flex flex-col gap-2.5 bg-slate-50/70 p-3.5 sm:p-4 rounded-2xl border border-slate-200/60 relative">
                      <button
                        type="button"
                        onClick={() => onRemoveVehicle(idx)}
                        className="absolute top-3 right-3 p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all"
                        title="Remove Vehicle"
                      >
                        <Trash2 size={15} />
                      </button>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3 pr-7 sm:pr-8">
                        <input type="text" className="input bg-white text-xs py-2 px-3 border-gray-200/50 shadow-xs" placeholder="Make (e.g. BMW)" value={v.make || ''} onChange={e => { const nv = [...formVehicles]; nv[idx] = { ...nv[idx], make: e.target.value }; setFormVehicles(nv); }} />
                        <input type="text" className="input bg-white text-xs py-2 px-3 border-gray-200/50 shadow-xs" placeholder="Model" value={v.model || ''} onChange={e => { const nv = [...formVehicles]; nv[idx] = { ...nv[idx], model: e.target.value }; setFormVehicles(nv); }} />
                        <input type="text" className="input bg-white text-xs py-2 px-3 border-gray-200/50 shadow-xs uppercase placeholder-normal" placeholder="Plate No." value={v.plate || ''} onChange={e => { const nv = [...formVehicles]; nv[idx] = { ...nv[idx], plate: e.target.value }; setFormVehicles(nv); }} />
                        <select
                ...
                <button type="button" onClick={() => setFormVehicles([...formVehicles, { id: null, make: '', model: '', plate: '', vehicle_type_id: null, type: '' }])} className="text-[12px] font-bold text-blue-600 flex items-center gap-1.5 hover:text-blue-700 hover:bg-blue-50 px-3.5 py-2 rounded-xl transition-colors mt-2 inline-flex bg-blue-50/40 border border-blue-200/50">
