...
export default function MasterService() {
  ...
  const { data: services = [], isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return [
        { _id: 's1', name: 'Premium Exterior Wash', description: 'Foam wash, alloy wheel cleaning, and tire dressing.', price: 999, isActive: true },
        { _id: 's2', name: 'Interior Vacuum & Wipe', description: 'Deep vacuum of seats and carpets, dashboard polishing.', price: 799, isActive: true },
        { _id: 's3', name: 'Engine Bay Detailing', description: 'Complete degreasing and dressing of engine bay components.', price: 1499, isActive: true },
        { _id: 's4', name: 'Headlight Restoration', description: 'Restoring yellowed or cloudy headlights to clear finish.', price: 1199, isActive: false }
      ];
    },
    staleTime: 5 * 60 * 1000
  });
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredServices.map(srv => (
          <div key={srv._id} onClick={() => handleEdit(srv)} className="card p-5 relative overflow-hidden flex flex-col cursor-pointer group hover:-translate-y-1 transition-all duration-300">
            <div className="flex justify-between items-start mb-2">
              <h3 className="text-lg font-bold text-gray-900 pr-24 group-hover:text-blue-600 transition-colors">{srv.name}</h3>
              {/* Sparkline Chart mimicking the image */}
              <div className="absolute top-5 right-5 flex flex-col items-end">
                ...
                <svg width="64" height="24" viewBox="0 0 64 24" className="opacity-90">
                  <defs>
                    <linearGradient id={`grad-${srv._id}`} x1="0" y1="0" x2="0" y2="1">
                  ...
                  <path d="M0,18 Q8,8 16,14 T32,10 T48,16 T64,6 L64,24 L0,24 Z" fill={`url(#grad-${srv._id})`} />
                </svg>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mt-auto">
              ...
              <button
                onClick={(e) => { e.stopPropagation(); handleStatusChange(srv._id, srv.isActive ? 'Inactive' : 'Active'); }}
